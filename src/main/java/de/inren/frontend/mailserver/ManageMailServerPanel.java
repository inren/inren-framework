 ...
 * 
 */
public class ManageMailServerPanel extends ManagePanel implements IAdminPanel {
    ...
    public ManageMailServerPanel(String id, IWorktopManageDelegate<MailServer> delegate) {
        super(id);
    ...
    protected final Component getTable(final String id) {
        ...
        Component table = builder.addDataProvider(new RepositoryDataProvider<MailServer>(mailServerRepository))
                    ...
                    public void populateItem(Item<ICellPopulator<MailServer>> cellItem, String componentId, IModel<MailServer> rowModel) {
                        ...
                        linkBuilder.add(new EditActionLink(true) {
                            ...
                            public void onClick(AjaxRequestTarget target) {
                                delegate.switchToComponent(target, delegate.getEditPanel(new Model<MailServer>(mailServer)));
                            }
                        });
                ...
                }).addPropertyColumn("id", true).addPropertyColumn("name", true).addPropertyColumn("host", true).addPropertyColumn("port", true)
                .addPropertyColumn("username", true).addPropertyColumn("password", true).addBooleanPropertyColumn("current").setNumberOfRows(10).build(id);
    ...
    protected Component getActionPanel(String id) {
        ...
        ButtonGroup bg = new ButtonGroup(id) {
            ...
            protected List<AbstractLink> newButtons(String buttonMarkupId) {
                List<AbstractLink> res = new ArrayList<AbstractLink>();
                StringResourceModel srm = new StringResourceModel("actions.create.mailServer", ManageMailServerPanel.this, null);

 ...
 * 
 */
@MountPath(value = "/categoryFilters")
public class ManageCategoryFiltersPage extends SecuredPage<Group> {

    @Override
    public Component createPanel(String wicketId) {
        final WorktopPanel w = new WorktopPanel(wicketId);
        w.setDelegate(new CategoryFilterWorktopManageDelegate(w));
        return w;
    }
}